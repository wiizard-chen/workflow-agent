...
一个 [pi/omp coding-agent](https://pi.dev) 扩展,把"需求 → PRD → 拆 task → 分配 dev → 测试"做成 **PRD / 执行 双模式**流水线。PRD 模式你和 omp 讨论需求;执行模式 omp 启动一个**技术经理**(独立 session),它把 PRD 拆成 task、分配给 **omp dev subagent**(`omp --print --model deepseek-flash --system-prompt <dev.md> --cwd <worktree>`,由 DevPool `spawn` 出来)、最后让 glm-5.2 测试,失败自动建 bd bug。
...
执行模式(/execute):
  主 omp spawn 一个经理 omp 进程(独立 session,加载 .omp/agents/manager.md):
    ...
    3. assign_dev(taskId, devId) → DevPool spawn 一个 omp dev subagent
       (`omp --print --model deepseek-flash --system-prompt <dev.md> --cwd <worktree>`)
       独立 task 并行散给不同 dev;dev 内部闭环验证(write→verify→fix 直到通过)
    4. run_test → glm-5.2 测试产出,blocker 创建为 bd bug
    5. bug 再分配 assign_dev 修复 → 重测,直到无 bug
  经理进程退出 → 主 session 汇报
```

**核心设计**:调度是经理 LLM 的判断(不是代码循环);每个 dev 是一个 omp native subagent,持有固定 worktree。跨 task 的上下文不再靠 session 续跑(原 reasonix 的 `--continue` 机制已移除),而是靠:(a) `cache.ts` 把 system-prompt 里的 date 冻结成常量,让 DeepSeek 服务端前缀缓存跨 task 保持热度;(b) bd comments 显式携带跨 task 状态。独立 task 并行派发给不同 dev,取代旧的"依赖链给同一 dev 走 --continue"路由。
...
| 实现每个 task | **omp dev subagent**(`omp --print`,by DevPool) | deepseek-flash |
...
- `pi`(v0.80+)、`bd`/`beads`(v1.1.0+)、`git` 已安装。(dev 执行层由 omp 原生 subagent 承担,无需额外二进制。)
  ```bash
...
不想手动一个个装?克隆仓库后跑一行,装齐 omp + beads、配好 API key、装好 skill:
...
5. `/execute` — 进入**执行模式**:主 omp 启动经理进程,它读 PRD → 拆 task 进 bd → 分配 omp dev subagent → 测试。失败自动建 bd bug,经理继续分配修复,直到全过。经理进程退出后回到主 session。
...
仓库还在 `skills/` 下提供四个针对流水线的 beads skill(omp 主 session 与 dev subagent 通用):

| skill | 用途 | 主要使用者 |
|---|---|---|
| `bd-plan` | 需求 → beads epic + PRD | 主 session omp |
| `bd-split` | PRD → task + 依赖(经理拆分) | 经理 omp 进程 |
| `bd-work` | 认领/实现/关闭单个 task | omp dev subagent(高频) |
| `bd-handoff` | 跨 session 交接(进度写 bd 不写本地) | 经理 + dev |

`pi install` 装的是**扩展**(`/wf`、`/execute` 等命令),skill 默认只在项目目录内可用。要让 **omp 在任何目录**(含经理/dev subagent)都加载这些 skill,装到全局 skill 根。
...
`workflow.config.json` 的 `execute.maxParallel` 是**经理可用的 dev 池大小**(默认 3)。每个 dev 是一个 omp native subagent,持有固定 worktree:

- 独立的 task 并行派发给不同 dev(取代旧的"依赖链给同一 dev 走 `--continue`"路由)。
- 跨 task 上下文不再靠 session 续跑,而是靠:(a) `cache.ts` 冻结 system-prompt 的 date 让 DeepSeek 前缀缓存跨 task 保持热度;(b) bd comments 显式携带跨 task 状态。
- dev 在自己的 omp subagent 内部做闭环验证(write → verify → fix 直到通过),再交回经理 `run_test`。
- 经理是驻留 LLM,默认做 stage 级委派(dynamic granularity),异常时细粒度介入。

**缓存安全性已实测验证**(详见 `DECISION_LOG.md`):worktree 路径不破坏 DeepSeek 前缀缓存。

## 验证门(P0 安全)

`/build` 默认**严格验证**:如果没有设置验证命令(`/wf verify <cmd>` 或配置 `build.verifyCommand`),每个子任务会因验证门为空而失败。这避免了 omp dev subagent 自主写入无任何校验直接 commit 的安全风险(dev 内部已做闭环 verify→fix,P0 门控兜底)。首次 `/build` 会在无验证命令时弹出确认,允许你显式接受"无验证"或先去设置。
...
└── <reqId>/
    ...
    ├── results/
    │   ├── NN.metrics.json     # 每个 omp dev subagent run 的 token/cache/cost
...
`workflow.config.json` 可调:provider endpoint / 模型 id、各阶段角色、**dev 执行层**(dev 模型 id / 超时,config 字段 `reasonix: {bin, model, maxSteps, timeoutMs}` 已迁移为 `dev: {model, timeoutMs}`)、默认验证命令、commit 前缀、**并行上限**(`execute.maxParallel`)。改完 `/reload` 或重启 pi。

## 缓存说明

- 每个子任务由一个独立的 omp dev subagent run 承担(`omp --print`),命中 DeepSeek **服务端前缀热缓存**(成本大头)。
- **worktree 并行不击穿缓存**(实测):worktree 的 cwd 差异不注入 omp system prompt,前缀缓存跨 worktree 共享。详见 `DECISION_LOG.md`。
- omp 侧多模型切换(pro/glm)天然会各走各的缓存桶,属于预期,量小无碍。
...
- **dev subagent 执行层**:cache.ts 同样覆盖——每个 omp dev subagent 启动时 system-prompt 的 date 被冻结成同一常量,前缀缓存跨 task / 跨 dev 保持热度(取代了原 reasonix 执行层自带的字节级缓存优化,~99.8% 命中率被保留)。
...
- omp dev subagent autonomous(不卡审批)但仍遵守 omp 的 `deny`(默认已拦 `git push`、`rm -rf`),sandbox 限写在目标 repo 内。
...
- 每个 omp dev subagent 是独立的一次 `omp --print` run;若网络受限访问 z.ai,给 omp 配好代理可减少 `Connection error` 重试。
